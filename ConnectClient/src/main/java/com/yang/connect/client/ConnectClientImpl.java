...
public class ConnectClientImpl implements ConnectClient {
	...
	/**
	 * Note: This constructor sets system-wide cookie handler SHARED BY ALL
	 * CONNECTIONS.
	 */
	private ConnectClientImpl() {
		...
		// *************** Sets system-wide cookie handler SHARED BY ALL
		// CONNECTIONS ****************
	...
	// an input stream.
	/*
	 * protected InputStream sendPOST(String urlString, Map<String, Object>
	 * dataMap) throws IOException {
	 * LogUtils.info(String.format("*** POST to URL %s ***", urlString));
	 * LogUtils.debug("0. Show cookie store"); displayCookieStore();
	 * 
	 * URL url = new URL(urlString); HttpURLConnection con = (HttpURLConnection)
	 * url.openConnection(); con.setReadTimeout(20000 milliseconds );
	 * con.setConnectTimeout(5000 milliseconds ); con.setRequestMethod("POST");
	 * con.setDoInput(true);
	 * 
	 * LogUtils.debug("1. Show cookie store"); displayCookieStore();
	 * 
	 * // Starts the query con.connect();
	 * LogUtils.debug("2. Show cookie store"); displayCookieStore();
	 * 
	 * int response = con.getResponseCode(); LogUtils.debug("The response is: "
	 * + response); LogUtils.debug("3. Show cookie store");
	 * displayCookieStore();
	 * 
	 * InputStream stream = con.getInputStream();
	 * 
	 * LogUtils.debug("4. Show cookie store"); displayCookieStore(); return
	 * stream; }
	 */
	...
	protected String sendPOST2(String urlString, Map<String, Object> dataMap) throws IOException {
		// By default, new instances of HttpCookie work only with servers that
		// support RFC 2965 cookies. Many web servers support only the older
		// specification, RFC 2109. For compatibility with the most web servers,
		 ...
		 * the connection pool. Work around this by disabling connection
		 * pooling: private void disableConnectionReuseIfNecessary() { // Work
		 * around pre-Froyo bugs in HTTP connection reuse. if
		...
		// what are they?
		final boolean autoRedirect = true;
	...
	private void redirectManually(int responseCode, URL url, HttpURLConnection conn) throws IOException {
		...
		if (responseCode == 301 || responseCode == 302 || responseCode == 303) {
			...
			LogUtils.info(String.format("******* WE ARE REDIRECTED from %s to %s. ********* ", originalHost,
					respondedHost));
	...
	/**
	 * Converts parameter map to post data string with specified encoding, which
	 * is ready to send over wire via http POST. Note: the encoding is handled
	 * as UTF-8.
	 * 
	 * @param params
